...
public class AtlasImageEditor : ImageEditor
{
    ...
    public void DrawAtlasPopupLayout()
    {
        using (new EditorGUILayout.HorizontalScope())
        {
            ...
            SpriteAtlas atlas = _spAtlas.objectReferenceValue as SpriteAtlas;
            ...
            if (GUI.Button(rect, atlas ? new GUIContent(atlas.name) : Txt_SpriteAtlasNull, EditorStyles.popup))
            {
                ...
                {
                    _spAtlas.objectReferenceValue = obj;
                    _spAtlas.serializedObject.ApplyModifiedProperties();
                });
            }
            if (atlas != null)
            {
    ...
    protected static void ConvertTo<T>(Object context) where T : MonoBehaviour
    {
        ...
        MonoBehaviour obj = (so.targetObject as MonoBehaviour);
        obj.enabled = true;
        obj.enabled = oldEnable;
    }
}
