...
public class WorldCreateHelper
{
    ...
    {
        MapManager mapMgr = GameManager<MapManager>.QGetOrAddMgr();
        int halfX = mapMgr.FindPath.MapInfo.MapSizeX / 2;
        int halfZ = mapMgr.FindPath.MapInfo.MapSizeY / 2;

        for (int cnt = 0; cnt < 20; cnt++)
        {
            Vector3 pos = mapMgr.FindPath.CenterPosition;
            Vector3 newPos = pos;
            if (cnt != 0)
            {
                newPos = GameHelper.GetRangePosition(pos, -halfX, halfX, -halfZ, halfZ);
            }
