...
public class SceneVariants
{
    ...
    //随机创建一个地图 
    public static void RandomMap(int mapWidth, int mapHeight, float waterline = 5.00f)
    {
        ...
        for (var i = 0; i < mapWidth; i++)
        {
            for (var j = 0; j < mapHeight; j++)
            {
                ...
                float pValue = Mathf.PerlinNoise(i * 1.0f / mapWidth, j * 1.0f / mapHeight);
                pValue = pValue * Random.Range(10.00f, 20.00f);
                //Debug.LogFormat("{0} {1} ------ {2}", i, j, pValue);
